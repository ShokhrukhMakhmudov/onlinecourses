...
export default function page() {
  ...
  useEffect(() => {
    async function fetchCourses() {
      ...
      setLoading(true);
      try {
        const req = await fetch(`/api/users/purchase/get?id=${userData._id}`, {
          method: "GET",
        });
  ...
  return (
    <main className="mt-10">
      <div className="container">
        <h1 className="text-2xl font-semibold mb-5">Mening kurslarim:</h1>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 place-items-center">
          {userData &&
            courses.map(({ _id, title, author, cover, language, duration }) => (
              <div className="card" key={_id as string}>
                <figure>
                ...
                <div className="card-body p-4 gap-3">
                  <Link
                    ...
                    className="card-title font-normal text-primary ">
                    {title}
                  </Link>
